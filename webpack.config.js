; (function(require) {
    ...
    if (isProd || MIN_VERSION) {
        ...
        webpackConfigTemplate.devtool = 'eval';
    }

    if (!RUNNING_SERVER || !isProd) {
        webpackConfigTemplate.entry[fileBuildName] = [path.resolve(__dirname, 'src/main.js')];
    ...
    if (RUNNING_SERVER && webpackConfigTemplate.entry.ue) {
        webpackConfigTemplate.entry.ue.unshift('webpack-dev-server/client?https://' + localHost + ':' + freePort + '/');
